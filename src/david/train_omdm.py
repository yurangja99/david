...
from imports.genpose.datasets.datasets_genpose_RT import get_data_loaders_from_cfg, process_batch
from imports.genpose.networks.posenet_agent_RT import PoseNet 
from imports.genpose.configs.config import get_config
from imports.genpose.utils.misc import exists_or_mkdir
from imports.genpose.utils.genpose_utils import merge_results
from imports.genpose.utils.misc import average_quaternion_batch, parallel_setup, parallel_cleanup
from imports.genpose.utils.metrics import get_metrics, get_rot_matrix
from imports.genpose.utils.so3_visualize import visualize_so3
from imports.genpose.utils.visualize import create_grid_image
import wandb
wandb.login()
...
def train_score(cfg, train_loader, score_agent, teacher_model=None):
    ...
    for epoch in range(score_agent.clock.epoch, cfg.n_epochs):
        ...
        if score_agent.clock.epoch % cfg.eval_freq == 0:   
            ...
            for i in range(len(data_modes)):
                ...
                res_list, sampler_mode_list = score_agent.eval_func(test_batch, data_mode)
                samples_dir = f"./results/samples/{cfg.log_dir}"
                ...
                for i, sampler_mode in enumerate(sampler_mode_list):
                    ...
                    samples[sampler_mode] = {"base_R": base_rot_mat, "base_t": base_trans}
            
                with open(f'{samples_dir}/{score_agent.clock.epoch}.pkl', 'wb') as f:
