...
from imports.genpose.networks.pts_encoder.pointnets import PointNetfeat
from imports.genpose.networks.pts_encoder.pointnet2 import Pointnet2ClsMSG, ThetaEncoder
# from imports.genpose.networks.pts_encoder.pointnet2 import ThetaEncoder
from imports.genpose.networks.gf_algorithms.samplers import cond_ode_likelihood, cond_ode_sampler, cond_pc_sampler, cond_ode_sampler_for_RT, cond_ode_sampler_for_RT_inference
# from imports.genpose.networks.gf_algorithms.samplers import cond_ode_likelihood, cond_pc_sampler, cond_ode_sampler_for_RT
from imports.genpose.networks.gf_algorithms.scorenet_RT import PoseScoreNet, PoseDecoderNet
# from imports.genpose.networks.gf_algorithms.scorenet_RT import PoseScoreNet
from imports.genpose.networks.gf_algorithms.energynet import PoseEnergyNet
...
class GFObjectPose(nn.Module):
    def __init__(self, cfg, prior_fn, marginal_prob_fn, sde_fn, sampling_eps, T):
        ...
        ''' encode theta '''
        self.theta_encoder = ThetaEncoder(theta_dim=21 * 3, out_dim=512) # SMPLX theta dimension
    ...
    def sample(self, data, sampler, atol=1e-5, rtol=1e-5, snr=0.16, denoise=True, init_x=None, T0=None):
        if sampler == 'pc':
            ...
            in_process_sample, res =  cond_ode_sampler_for_RT(
                score_model=self,
                data=data,
                prior=self.prior_fn,
                sde_coeff=self.sde_fn,
        ...
        elif sampler == 'ode_inference':
            T0 = self.T if T0 is None else T0
            in_process_sample, res =  cond_ode_sampler_for_RT_inference(
                score_model=self,
                data=data,
                prior=self.prior_fn,
                sde_coeff=self.sde_fn,
    ...
    def forward(self, data, mode='score', init_x=None, T0=None):
        ...
        if mode == 'score':
            ...
            return in_process_sample, res
        elif mode == 'ode_inference_sample':
            in_process_sample, res = self.sample(data, 'ode_inference', init_x=init_x, T0=T0)
            return in_process_sample, res
        elif mode == 'theta':
            theta_feature = self.extract_theta_feature(data)
            return theta_feature
        else:
            raise NotImplementedError
