...
from imports.genpose.networks.gf_algorithms.score_utils import ExponentialMovingAverage
from imports.genpose.networks.gf_algorithms.losses_RT import loss_fn, loss_fn_edm
from imports.genpose.networks.gf_algorithms.sde import init_sde
from imports.genpose.networks.posenet_RT import GFObjectPose
...
from imports.genpose.networks.reward import sort_results, ranking_loss, sort_poses_by_energy
from imports.genpose.utils.genpose_utils import TrainClock
from imports.genpose.utils.misc import exists_or_mkdir, average_quaternion_batch
from imports.genpose.utils.visualize import create_grid_image, test_time_visulize
from imports.genpose.utils.metrics import get_metrics, get_rot_matrix


 
def get_ckpt_and_writer_path(cfg):
        if cfg.use_pretrain and not os.path.exists(f"./{cfg.log_dir}/ckpt_epoch{cfg.model_name}.pth"):
            raise Exception(f"./{cfg.log_dir}/ckpt_epoch{cfg.model_name}.pth is not exist!")
        
        ''' init exp folder and writer '''
        ckpt_path = f'./{cfg.log_dir}'
        writer_path = f'./cache/omdm/{cfg.log_dir}' if cfg.use_pretrain == False else f'./cache/omdm/{cfg.log_dir}_continue'
...
class PoseNet(nn.Module):
    ...
    def train_score_func(
        ...
        self.pts_feature = True
        gf_losses = self.collect_score_loss(
            data,
            # teacher_model,
            # pts_feat_teacher
        )
    ...
    def train_func(self, data, pose_samples=None, gf_mode='score'):
        if gf_mode in ['score', 'energy_wo_ranking']:
            losses = self.train_score_func(
                data,
                # teacher_model
            )
        elif gf_mode == 'energy':
    ...
    def eval_score_func(self, data, data_mode):
        ...
        return res_list, sampler_mode_list
    
    def inference_score_func(
            self, 
            batch_size,
            thetas,
            human_vertices,
        ...
        with torch.no_grad():
            ...
            data["contact_threshold"] = 0.05 if contact_threshold is None else contact_threshold
            
            data["human_vertices"] = torch.tensor(human_vertices).to('cuda').float()
            if type(object_scale) != float:
                data["object_vertices"] = torch.from_numpy(np.array([obj_scale * object_vertices for obj_scale in object_scale])).to('cuda').float()
            else:
                data["object_vertices"] = object_scale * torch.tensor(object_vertices).to('cuda').float()
            data["ratio"] = ratio
    ...
    def eval_func(self, data, data_mode, pose_samples=None, gf_mode='score'):
        if gf_mode in ['score', 'energy_wo_ranking']:
            ...
            res_list, sampler_mode_list = self.eval_score_func(data, data_mode)
            return res_list, sampler_mode_list
        elif gf_mode == 'energy':
